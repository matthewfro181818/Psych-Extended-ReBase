...
class CharacterEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create()
	{
		...
		var tipText:FlxText = new FlxText(FlxG.width - 300, FlxG.height - 24, 300, 'Press ${!useDesktopThings ? 'F' : 'F1'} for Help', 20);
	...
	{
		var str:Array<String> = (!useDesktopThings) ? [
			"CAMERA",
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.keys.pressed.SHIFT #if TOUCH_CONTROLS || mobilePad.buttonC.pressed #end)
		{
			shiftMult = 4;
			shiftMultBig = 10;
		}
		...
		if(FlxG.keys.justPressed.R && !FlxG.keys.pressed.CONTROL #if TOUCH_CONTROLS || mobilePad.buttonZ.justPressed #end) FlxG.camera.zoom = 1;
		else if ((FlxG.keys.pressed.E #if TOUCH_CONTROLS || mobilePad.buttonX.pressed #end) && FlxG.camera.zoom < 3) {
			FlxG.camera.zoom += elapsed * FlxG.camera.zoom * shiftMult * ctrlMult;
			if(FlxG.camera.zoom > 3) FlxG.camera.zoom = 3;
		}
		else if ((FlxG.keys.pressed.Q #if TOUCH_CONTROLS || mobilePad.buttonY.pressed #end) && FlxG.camera.zoom > 0.1) {
			FlxG.camera.zoom -= elapsed * FlxG.camera.zoom * shiftMult * ctrlMult;
		...
		if(anims.length > 1)
		{
			...
			else if((FlxG.keys.justPressed.S #if TOUCH_CONTROLS || mobilePad.buttonD.justPressed #end) && (changedAnim = true)) curAnim++;
			...
			if(changedAnim)
			{
				...
				curAnim = FlxMath.wrap(curAnim, 0, anims.length-1);
				character.playAnim(anims[curAnim].anim, true);
				//reload saved positions (Idk why editor not loading changed position
				character.offset.x = anims[curAnim].offsets[0];
				character.offset.y = anims[curAnim].offsets[1];
				updateText();
			}
		}
		...
		#if TOUCH_CONTROLS
		if (mobilePad.buttonA.justPressed)
		{
			undoOffsets = [character.offset.x, character.offset.y];
			character.offset.x = copiedOffset[0];
			character.offset.y = copiedOffset[1];
		...
		else if(FlxG.keys.justPressed.ESCAPE #if TOUCH_CONTROLS || mobilePad.buttonB.justPressed #end)
		{
			if(!_goToPlayState)
			{
				if(!unsavedProgress)
