...
class ChartingStateNew extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create()
	{
		...
		var tipText:FlxText = new FlxText(FlxG.width - 210, FlxG.height - 30, 200, 'Press ${(usingMobile) ? 'F' : 'F1'} for Help', 20);
	...
	override function update(elapsed:Float)
	{
		...
		var holdingAlt:Bool = #if TOUCH_CONTROLS mobilePad.buttonG.justPressed || #end FlxG.keys.pressed.ALT;
		if(FlxG.sound.music != null)
		{
			if(PsychUIInputText.focusOn == null) //If not typing anything
			{
				if(#if TOUCH_CONTROLS mobilePad.buttonC.justPressed || #end FlxG.keys.justPressed.F12)
				{
				...
				else if((#if TOUCH_CONTROLS mobilePad.buttonLeft.justPressed != mobilePad.buttonRight.justPressed || #end FlxG.keys.justPressed.A != FlxG.keys.justPressed.D) && !holdingAlt)
				{
					...
					var shiftAdd:Int = (#if TOUCH_CONTROLS mobilePad.buttonY.pressed || #end FlxG.keys.pressed.SHIFT ? 4 : 1);

					if(#if TOUCH_CONTROLS mobilePad.buttonLeft.justPressed || #end FlxG.keys.justPressed.A)
					{
					...
					else if(#if TOUCH_CONTROLS mobilePad.buttonRight.justPressed || #end FlxG.keys.justPressed.D)
					{
				...
				else if((#if TOUCH_CONTROLS mobilePad.buttonUp.pressed != mobilePad.buttonDown.pressed || #end FlxG.keys.pressed.W != FlxG.keys.pressed.S) || FlxG.mouse.wheel != 0)
				{
					...
					if(mouseSnapCheckBox.checked && FlxG.mouse.wheel != 0)
					{
						...
						var timeAdd:Float = (#if TOUCH_CONTROLS mobilePad.buttonY.pressed || #end FlxG.keys.pressed.SHIFT ? 4 : 1) / (holdingAlt ? 4 : 1) * -FlxG.mouse.wheel * snap;
					...
					{
						var speedMult:Float = (#if TOUCH_CONTROLS mobilePad.buttonY.pressed || #end FlxG.keys.pressed.SHIFT ? 4 : 1) * (FlxG.mouse.wheel != 0 ? 4 : 1) / (holdingAlt ? 4 : 1);
						if(#if TOUCH_CONTROLS mobilePad.buttonUp.pressed || #end FlxG.keys.pressed.W || FlxG.mouse.wheel > 0)
							FlxG.sound.music.time -= Conductor.crochet * speedMult * 1.5 * elapsed / curZoom;
						else if(#if TOUCH_CONTROLS mobilePad.buttonDown.pressed || #end FlxG.keys.pressed.S || FlxG.mouse.wheel < 0)
							FlxG.sound.music.time += Conductor.crochet * speedMult * 1.5 * elapsed / curZoom;
					}
				...
				else if(#if TOUCH_CONTROLS mobilePad.buttonX.justPressed || #end FlxG.keys.justPressed.SPACE)
				{
		...
		if(PsychUIInputText.focusOn == null && lastFocus == null)
		{
			...
			if(#if TOUCH_CONTROLS mobilePad.buttonA.justPressed || #end FlxG.keys.justPressed.ENTER)
			{
				goToPlayState();
				return;
			}
		...
		if (usingMobile #if TOUCH_CONTROLS && mobilePad.inState == true #end)
		{
				...
				if(touch.x >= minX && touch.x < gridBg.x + gridBg.width)
				{
					...
					else if(touch.justPressed && !ignoreClickForThisFrame)
					{
						...
						else if(touch.x >= gridBg.x && touch.x < gridBg.x + gridBg.width)
						{
							...
							{
								if(#if TOUCH_CONTROLS mobilePad.buttonH.pressed || #end holdingAlt) // Select Note/Event
								{
									var sel = selectedNotes.copy();
									...
									else if(#if TOUCH_CONTROLS !mobilePad.buttonH.pressed || #end !holdingAlt)
									{
										resetSelectedNotes();
		...
		if(selectedNotes.length > 0)
		{
			...
			var qPress = (#if TOUCH_CONTROLS mobilePad.buttonUp2.justPressed || #end FlxG.keys.justPressed.Q);
			var ePress = (#if TOUCH_CONTROLS mobilePad.buttonDown2.justPressed || #end FlxG.keys.justPressed.E);
			var addSus = (#if TOUCH_CONTROLS mobilePad.buttonY.pressed || #end FlxG.keys.pressed.SHIFT ? 4 : 1) * (Conductor.stepCrochet / 2);
			if(qPress) addSus *= -1;
	...
	function addFileTab()
	{
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Preview (${(usingMobile) ? 'C' : 'F12'})', openEditorPlayState, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Playtest (${(usingMobile) ? 'A' : 'ENTER'})', goToPlayState, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
	...
	{
		#if TOUCH_CONTROLS mobilePad.inState = false; #end
	...
	{
		#if TOUCH_CONTROLS mobilePad.inState = true; #end
		ClientPrefs.toggleVolumeKeys(true);
		super.closeSubState();
		upperBox.isMinimized = true;
