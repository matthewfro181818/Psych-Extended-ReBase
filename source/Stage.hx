...
class Stage extends FlxBasic implements IBeatReceiver {
	...
	**/
	...
	public function new(stage:String, ?state:FlxState, autoLoad:Bool = true) {
		...
		trace(stagePath);
		try if (FileSystem.exists(stagePath)) stageXML = new Access(Xml.parse(File.getContent(stagePath)).firstElement())
		catch(e) trace('Couldn\'t load stage "$stageFile": ${e.message}');
	...
	public function loadXml(xml:Access, forceLoadAll:Bool = false) {
		...
		if (xml != null) {
			...
			for(node in xml.elements) {
				if (node.name == "high-memory" && (!ClientPrefs.data.lowQuality || forceLoadAll)) for(e in node.elements) __pushNcheckNode(elems, e);
				else if (node.name == "low-memory" && (ClientPrefs.data.lowQuality || forceLoadAll)) for(e in node.elements) __pushNcheckNode(elems, e);
	...
	**/
	public function applyCharStuff(char:Character_CNE, posName:String, id:Float = 0) {
...
class StageCharPos extends FlxObject {
	...
	public function prepareCharacter(char:Character_CNE, id:Float = 0) {
		_id = id;
	...
	public function getOldInfo(char:Character_CNE) {
		return {
			x: char.x, y: char.y,
	...
	public function revertCharacter(char:Character_CNE) {
