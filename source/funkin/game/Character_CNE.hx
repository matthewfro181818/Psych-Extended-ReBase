...
@:allow(PlayState)
class Character_CNE extends FunkinSprite implements IBeatReceiver implements IOffsetCompatible implements IPrePostDraw {
	...
	public var gameOverCharacter:String = Character_CNE.FALLBACK_DEAD_CHARACTER;
	...
	public function applyXML(xml:Access) { // just for now till i remake the dumb editor
		gameOverCharacter = Character_CNE.FALLBACK_DEAD_CHARACTER;
	...
	public inline function buildXML(?animsOrder:Array<String>):Xml {
		...
		if (gameOverCharacter != Character_CNE.FALLBACK_DEAD_CHARACTER) xml.set("gameOverChar", gameOverCharacter);
	...
	public static function getXMLFromCharName(character:OneOfTwo<String, Character_CNE>):Access {
		var char:Character_CNE = null;
		if (character is Character_CNE) {
			char = cast character;
	...
	*/
	...
	/* Things taken from PsychEngine's Character.hx for fixing the problems between two classes */
	public var positionArray:Array<Float> = [0, 0]; //doesn't do anything, just there for fixing the issues
